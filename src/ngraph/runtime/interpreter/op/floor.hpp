...
template <typename T>
class ngraph::runtime::interpreter::FloorExec : public ExecNode<T>
{
public:
    static ExecNode<T> create(const ngraph::Node* node)
    {
        std::cout << "create Floor" << std::endl;
        return FloorExec(node);
    }

    FloorExec(const ngraph::Node* node)
        : m_node{dynamic_cast<const ngraph::op::Floor*>(node)}
    {
