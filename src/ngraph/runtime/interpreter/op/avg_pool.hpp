...
template <typename T>
class ngraph::runtime::interpreter::AvgPoolExec : public ExecNode<T>
{
public:
    static ExecNode<T> create(const ngraph::Node* node)
    {
        std::cout << "create AvgPool" << std::endl;
        return AvgPoolExec(node);
    }

    AvgPoolExec(const ngraph::Node* node)
        : m_node{dynamic_cast<const ngraph::op::AvgPool*>(node)}
    {
...
private:
    const ngraph::op::AvgPool* m_node;
};
