...
template <typename T>
class ngraph::runtime::interpreter::ReverseSequenceExec : public ExecNode<T>
{
public:
    static ExecNode<T> create(const ngraph::Node* node)
    {
        std::cout << "create ReverseSequence" << std::endl;
        return ReverseSequenceExec(node);
    }

    ReverseSequenceExec(const ngraph::Node* node)
        : m_node{dynamic_cast<const ngraph::op::ReverseSequence*>(node)}
    {
...
private:
    const ngraph::op::ReverseSequence* m_node;
};
