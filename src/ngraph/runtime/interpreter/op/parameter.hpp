...
template <typename T>
class ngraph::runtime::interpreter::ParameterExec : public ExecNode<T>
{
public:
    static ExecNode<T> create(const ngraph::Node* node)
    {
        std::cout << "create Parameter" << std::endl;
        return ParameterExec(node);
    }

    ParameterExec(const ngraph::Node* node)
        : m_node{dynamic_cast<const ngraph::op::Parameter*>(node)}
    {
...
private:
    const ngraph::op::Parameter* m_node;
};
